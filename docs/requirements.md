...
| Logging | `Microsoft.Extensions.Logging` → rolling file in `ApplicationData.Current.LocalFolder` | |
...
| `System.Diagnostics.Metrics` | Transport instrumentation | Counters for transactions, timeouts, parse warnings. Surfaced in Diagnostics; not exported anywhere. |
...
- Persist trends to a SQLite file in `LocalFolder` so restarts do not lose history. `Microsoft.Data.Sqlite` is packaged-app safe.
