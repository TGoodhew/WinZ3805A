...
public static class DriftAdvisory
{
    ...
    {
        { TotalDays: >= 2 } => $"{span.TotalDays.ToString("N1", CultureInfo.CurrentCulture)} days",
        { TotalHours: >= 2 } => $"{span.TotalHours.ToString("N1", CultureInfo.CurrentCulture)} hours",
        { TotalMinutes: >= 2 } => $"{span.TotalMinutes.ToString("N0", CultureInfo.CurrentCulture)} minutes",
        _ => $"{span.TotalSeconds.ToString("N0", CultureInfo.CurrentCulture)} seconds",
    };
